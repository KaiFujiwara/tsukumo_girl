...
### プロジェクトの性質
- **コアコンセプト**: バーコードスキャン → 付喪神（美少女キャラ）顕現 → AI会話 → 好感度・恋人システム
- **世界観**: 付喪神思想 - モノに魂が宿るという日本の伝統的世界観の現代版
- **ゲーム性**: キャラクター収集、育成、恋人関係の競争
- **将来の拡張性**: ゲーム的な演出・機能の追加可能性あり

### 確定した方針
- **識別子**: JAN/EANコード（バーコード）を使用
- **キャラ生成**: 決定論的パターン生成（リアルタイムAI画像生成は使わない）
- **商品情報**: 取得しない（法的リスク最小化）
- **同一性保証**: 同じバーコード = 同じキャラ

### 技術的要求
- バーコードスキャン機能（必須）
- 決定論的キャラクター生成（パーツ組み合わせ）
- キャラクター表示・表情アニメーション
- AI会話UI（会話履歴の記憶）
- 好感度システム（数値管理・演出）
- 恋人関係の管理・移動ロジック
...
### 1. PNGレイヤー方式（パーツ組み合わせ）⭐ 推奨

**概要**: パーツごとのPNG画像を重ね合わせて表示

| 項目 | 評価 |
|------|------|
| **ライセンス** | 🟢 無料（ツール不要） |
| **決定論的生成との相性** | 🟢 完全対応（パーツ組み合わせ） |
| **実装難易度** | 🟢 非常に低い |
| **データサイズ** | 中（透過PNG 約700枚で開始可能） |
| **モバイル対応** | ✅ 完全対応 |
| **バリエーション** | 🟢 920,000通り以上 |
...
**生成方法**: バーコード → ハッシュ → 擬似乱数 → パーツ選択

**適用事例**: アバター生成システム、ソーシャルゲーム
